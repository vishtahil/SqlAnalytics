"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var core_1 = require("@angular/core");
var error_notify_service_1 = require("./error-notify-service");
var GlobalErrorHandler = (function () {
    function GlobalErrorHandler(errorNotifyService) {
        this.errorNotifyService = errorNotifyService;
    }
    GlobalErrorHandler.prototype.handleError = function (error) {
        var _this = this;
        var unwrappedError = error.originalError || error;
        setTimeout(function () {
            _this.errorNotifyService.error(unwrappedError);
        }, 1);
    };
    return GlobalErrorHandler;
}());
GlobalErrorHandler = __decorate([
    __param(0, core_1.Inject(error_notify_service_1.ErrorNotifyService)),
    __metadata("design:paramtypes", [error_notify_service_1.ErrorNotifyService])
], GlobalErrorHandler);
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = GlobalErrorHandler;
//# sourceMappingURL=global-error-handler.js.map